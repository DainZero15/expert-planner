...
export default async function EditExpertPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: expert }, { data: branchData }] = await Promise.all([
    db.from("experts").select("*").eq("id", id).maybeSingle(),
    db.from("branches").select("id,name,address_line,postal_code,city").order("name"),
  ]);
  if (!expert) notFound();
  const profile = expert.preferences && typeof expert.preferences === "object" ? expert.preferences as { skills?: string[]; notes?: string } : {};
  return <main className="shell"><header className="topbar"><Link className="brand" href="/dashboard">Expert <span>Planner</span></Link><AppNavigation /></header><form action="/api/experts" method="post" className="card"><div className="eyebrow">Expert beheren</div><h1>{expert.name} wijzigen</h1><ExpertFields branches={branchData ?? []} expert={{ id: expert.id, name: expert.name, start: String((expert.start_address as { address?: string })?.address || ""), end: String((expert.end_address as { address?: string })?.address || ""), startTime: String(expert.start_time).slice(0, 5), endTime: String(expert.end_time).slice(0, 5), breakMinutes: expert.break_minutes, visitMinutes: expert.default_visit_minutes, areas: expert.service_areas, excluded: expert.excluded_areas, skills: profile.skills, notes: profile.notes, startBranchId: expert.start_branch_id, lunchBranchId: expert.lunch_branch_id, endBranchId: expert.end_branch_id }} /><button type="submit">Wijzigingen opslaan</button></form></main>;
}
