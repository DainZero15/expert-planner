...
export default async function Customers() {
  ...
  return <main className="shell"><header className="topbar"><Link className="brand" href="/dashboard">Expert <span>Planner</span></Link><AppNavigation/><div className="header-links"><Link className="archive-link" href={"/customers/dubbelen" as never}>Dubbelen</Link><Link className="archive-link" href={"/customers/prullenbak" as never}>Prullenbak</Link></div></header><CustomerForm/><section className="card" style={{ marginTop: 20 }}><h1>Klanten</h1><p className="muted">{regular.length} unieke klanten. {duplicates.length ? `${duplicateRows} regels staan apart bij mogelijke dubbelen.` : "Geen dubbele naam- en adrescombinaties gevonden."}</p><div style={{ overflowX: "auto" }}><table><thead><tr><th>Klant</th><th>Adres</th><th>Contact</th><th>Prioriteit</th><th>Status</th><th /></tr></thead><tbody>{regular.map((customer) => <tr key={customer.id}><td>{customer.name}{customer.customer_number && <><br /><small>{customer.customer_number}</small></>}</td><td>{customerAddress(customer)}</td><td>{customer.phone || customer.email || "—"}</td><td>{customer.priority}</td><td>{customer.geocode_status === "pending" ? "Adres te controleren" : customer.status}</td><td><form action={removeCustomer.bind(null, customer.id)}><button className="danger">Naar prullenbak</button></form></td></tr>)}</tbody></table>{!regular.length && <p className="muted">Nog geen unieke klanten. Mogelijke dubbelen staan apart.</p>}</div></section></main>;
}
